...
import { MessageSquare, Sparkles, BookOpen, Globe, Palette, Zap, ChevronLeft, ChevronRight } from 'lucide-react';
import { Layers, Wrench, Settings, Link2, Fuel, Ship, Package } from "lucide-react";
...
const Products = () => {
  ...
  // Reset crousel when category change
  ...
  return (
    <section id='products' className='relative py-20 bg-black overflow-hidden'>
        ...
        <FadeIn delay={0}>
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-white/60 max-w-2xl mx-auto">
              Explore our most in-demand pipes, fittings, valves, and industrial products trusted for oil field, marine, and general industrial applications.
            </p>
          </div>
        </FadeIn>
        ...
        <FadeIn delay={100}>
          <div className='flex flex-wrap justify-center gap-3 mb-16'>
            {categories.map((category) => (
              ...
              >
                <div className={`absolute inset-0 rounded-full transition-all duration-300 ${activeCategory === category ? 'bg-primary/10 opacity-100' : 'bg-white/5 border border-white/10 group-hover:bg-white/10'}`}></div>

                <div className='relative flex items-center gap-2'>
                  {React.createElement(categoryIcons[category] || Sparkles, { className: "w-4 h-4" })}
            ...
            {filteredProducts.length > 3 && (
              <>
                <button
                  ...
                  className='flex absolute left-0 top-1/2 -translate-1/2 -translate-x-2 lg:-translate-x-4 items-center justify-center w-10 h-10 lg:w-12 lg:h-12 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full hover:bg-white/20 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed z-10'
